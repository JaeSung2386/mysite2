...
<body>
	<div id="container">
		...
		<div id="content">
			<div id="board">
				<form id="search" action="${pageContext.servletContext.contextPath }/board?a=search" method="post">
					...
					<tr>
						...
						<c:choose>
							<c:when test="${vo.o_no eq '1' }">
								<td style="text-align: left;"><a href="${pageContext.servletContext.contextPath }/board?a=view&no=${vo.no}&user_no=${authuser.no}">${vo.title }</a></td>
							</c:when>
							<c:when test="${vo.o_no != '1' }">
								<td style="text-align: left; padding:0px 0px 0px ${vo.depth * 20}px;"><img src ="${pageContext.servletContext.contextPath }/assets/images/reply.png" width=15 height=15 /><a href="${pageContext.servletContext.contextPath }/board?a=view&no=${vo.no}">[답글]${vo.title }</a></td>
							</c:when>
						</c:choose>
						<td>${vo.name }</td>
						<td>${vo.hit }</td>
						...
						<td>
							...
							<c:if test="${!empty authuser and authuser.no eq vo.user_no}">
								<a href="${pageContext.servletContext.contextPath }/board?a=deleteform&no=${vo.no}">
