			...
			<div id="board" class="board-form">
				...
				<c:forEach items="${list }" var="commentvo" varStatus="status">
					<tr>
						...
						<c:if test="${authuser.no != null}">
							<td>
								<a href="${pageContext.servletContext.contextPath }/comment?a=replyform&no=${commentvo.no}">답글</a>
							</td>
							<!-- 댓글을 작성한 유저만 수정 삭제 가능  -->
							<c:if test="${!empty authuser and authuser.no eq commentvo.user_no}">
							<td>
								<a href="${pageContext.servletContext.contextPath }/comment?a=modifyform&no=${commentvo.no}">수정</a>
							</td>
							<td>
								<a href="${pageContext.servletContext.contextPath }/comment?a=deleteform&no=${commentvo.no}">삭제</a>
							</td>
							</c:if>
						</c:if>
					</tr>
				...
				<div class="bottom">
					...
					<c:if test="${!empty authuser and authuser.no ne vo.user_no}">
						<a href="${pageContext.servletContext.contextPath }/board?a=replyform&no=${vo.no}">답글달기</a>
					</c:if>
				</div>
			</div>
		</div>
